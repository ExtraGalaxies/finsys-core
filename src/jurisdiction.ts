...
/**
 * The jurisdiction registry, in the package that every app shares.
 *
 * WHY IT LIVES HERE AND NOT IN finsys-api
 *
 * finsys-api has carried `JURISDICTION` since before this package existed, and that is where it
 ...
 * de-duplication is a follow-up, and is the reason this
...
/**
 * The platform predates jurisdictions and was Malaysia-only, so an absent jurisdiction
 ...
 * spec is authored data, and silently normalizing it would let two spellings
...
export function resolveJurisdiction(value: string | null | undefined): Jurisdiction | null {
  ...
  // helper returned Malaysia for '', then swapping finsys-api onto it would
...
/**
 * Why a form and a program are not compatible.
 *
...
/**
 * Is this form usable to submit to this program?
 *
 ...
 *
 * Consumed by FinHub (the manager tier) and @finsys/borrower-client
 * (the SDK tier). Raw HTTP callers are unguarded by design.
 *
 * ABSENCE resolves to Malaysia on both sides, via `resolveJurisdiction`, so
 * there is ONE definition of "absent" rather than several disagreeing on
 * null and ''. Note that an empty string is NOT absent: finsys-api fails closed on it, and
 ...
 * treating them as a match would let a pair of mistakes authorize each other.
 */
...
/**
 * The currency a jurisdiction's amounts are DISPLAYED in
 * when the value itself does not say.
 *
 * READ THIS BEFORE USING IT — it is narrower than it looks.
 *
 * A currency belongs to the OBSERVATION, not to a
 ...
 * accept. Measured against real id shapes:
 *
 ...
 * recognize is exactly the guess this replaces.
 */
 ...
 * Substituting a guess here is how a Vietnamese figure ends up labeled MYR,
