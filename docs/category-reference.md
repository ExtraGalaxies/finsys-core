...
- **Write contract (SYS-3728)** — every value a writer stores is checked by `validateAdapterExtraction` / `validateCanonicalFields`, and every table re-checks it before rendering. A `string` field has an effective **`maxLength`** (default 256; a field or list item holding longer printed text declares more, and only such long text may contain a newline or tab), may declare a full-match **`pattern`** (only where the writer provably normalizes the format) and **`jurisdictionPatterns`** (a pattern per jurisdiction, applied only under that jurisdiction — never defaulted to MY). A `list` has an effective **`maxItems`** (default 500). Kind **`currency`** is an ISO 4217 code from `CURRENCY_CODES`, normalized from the printed form with `normalizeCurrency` (a bare `$` is ambiguous and refused). No string may open with a structure (a first visible `{`, or a `[` that reads as a sequence, is never closed, or — in short text — parses as JSON or carries a JSON marker — after marks and invisibles, fullwidth lookalikes included, and inside a JSON string literal), hold a default-ignorable, format, private-use, unassigned or control character, hold a space other than U+0020, carry leading or trailing whitespace, open with a combining mark, stack more than two nonspacing marks, be blank, or be a **placeholder** (no letter and no number — `-`, `—`, `.` — or `N/A`, `nil`, `null`, `none`, `see attached`, …: a writer omits it). A number or money value is a finite number, never text, a count is a non-negative integer, money carries at most two decimals and no more than its currency's ISO 4217 minor units (`excess-precision` — refused, never rounded; VND none), and a declared `range` is enforced (every credit-bureau count is bounded by its INT column). A `format` (`date` / `year`) is calendar-checked AND plausible: not before 1900-01-01, not after the validator's clock (`now`, injectable) plus a day — or plus ten years for a field declaring `mayBeFuture` — and `notAfter` orders two dates. A list stored as JSON text is at most 1,000,000 characters, nested at most four deep (`malformed-list`), with no object repeating a key and no row that is empty (`empty-row`). An extraction with no value anywhere, or a period with none of its own, is `empty-instance`; a category's `requiredAnyOf` names fields an instance must carry one of (`missing-identity` for an identity group — a credit-bureau principal's name and identifier). A writer's snapshot collapses a line break in short text to one space and stores a long text's line breaks as "\n"; a short value may open with a bracketed name ("[ACME] SDN BHD") but not with anything that parses as JSON or carries a JSON marker. Across a whole extraction, a category with a `currency` field states one currency, and states one wherever it states money; a `currencySource` field describes a currency present in its scope, and `inferred` only ever names the strictly matched jurisdiction's own currency (`currency-source-mismatch`); `periodFields` holds a period's envelope dates and year to its own fields. Every exported validator judges a plain-data NFC snapshot; writers call `prepareExtractionForWrite` and persist the snapshot it returns (NFC, lists re-serialized). `normalizePrintedText` is the writer-side text rule. Spreadsheet formula injection is an export-encoding concern, not this contract's.
...
| `bankingApprovedApplications12m` | number | count | 0–2147483647 | Credit applications approved in the last 12 months, from the banking (CCRIS) summary. |
| `bankingPendingApplicationsCount` | number | count | 0–2147483647 | Credit applications pending, from the banking summary. |
| `bankingSpecialAttentionAccountsCount` | number | count | 0–2147483647 | Accounts under special attention, from the banking summary. |
| `bankingLegalActionsCount` | number | count | 0–2147483647 | Banking facilities with legal action taken, from the banking summary. |
| `bankingExistingFacilitiesCount` | number | count | 0–2147483647 | Existing banking facilities, from the banking summary. |
| `windingUpRecordsCount` | number | count | 0–2147483647 | Winding-up records in the bureau's own databank summary (company subjects). |
| `bankruptcyRecordsCount` | number | count | 0–2147483647 | Bankruptcy records in the bureau's own databank summary (individual and party subjects). |
| `legalSuitsCount` | number | count | 0–2147483647 | Legal suits in the bureau's own databank summary. |
| `tradeCreditReferencesCount` | number | count | 0–2147483647 | Trade / credit references in the bureau's own databank summary. |
| `enquiries12m` | number | count | 0–2147483647 | Enquiries made on the subject in the last 12 months. |
| `businessInterestsCount` | number | count | 0–2147483647 | Companies or businesses the subject holds an interest in. |
...
| `securedFacilitiesCount` | number | count | 0–2147483647 | Number of secured banking facilities. |
...
| `securedMaxInstallmentsInArrears12m` | number | count | 0–2147483647 | Highest number of installments in arrears on any secured facility in the last 12 months. |
| `unsecuredFacilitiesCount` | number | count | 0–2147483647 | Number of unsecured banking facilities. |
...
| `unsecuredMaxInstallmentsInArrears12m` | number | count | 0–2147483647 | Highest number of installments in arrears on any unsecured facility in the last 12 months. |
...
| `suitsAsDefendantCount` | number | count | 0–2147483647 | Number of legal suits with the subject as defendant. |
| `limitedDetailSuitsAsDefendantCount` | number | count | 0–2147483647 | Number of other known suits, with limited details, with the subject as defendant. |
| `suitsAsPlaintiffCount` | number | count | 0–2147483647 | Number of legal suits with the subject as plaintiff. |
| `windingUpAsDefendantCount` | number | count | 0–2147483647 | Number of winding-up actions against the subject (company subjects). |
| `windingUpAsPetitionerCount` | number | count | 0–2147483647 | Number of winding-up actions brought by the subject (company subjects). |
| `bankruptcyActionsCount` | number | count | 0–2147483647 | Number of bankruptcy actions (individual and party subjects). |
...
| `mgmtCurrency` | string | currency |  | The statement's reporting currency as an ISO 4217 code; normalized from the printed form (RM, MYR, ...) by the writer. Absent when the statement prints a form that names no single currency, and when it prints none on an application whose jurisdiction the writer may not infer from (see mgmtCurrencySource). |
| `mgmtCurrencySource` | string | enum |  | Where mgmtCurrency came from. `printed` when the statement prints it, as FinXtract returns it: in its currency heading, or as a currency prefix on its figures; `inferred` when it prints no currency anywhere and the application is Malaysian, so the writer stored MYR — shown so a reviewer knows the currency was not on the page. Absent on rows written before this field existed. |
