...
// ---------------------------------------------------------------------------
// SYS-2181: Standalone SSM v2 text fields (category 14)
// ---------------------------------------------------------------------------

describe("SSM v2 standalone text fields (SYS-2181)", () => {
  const SSM_V2_FIELDS = [
    { name: "ssmCompanyName", displayName: "Company Name (SSM Profile)" },
    { name: "ssmCompanyRegNo", displayName: "Company Registration No (SSM Profile)" },
    { name: "ssmIncorporatedDate", displayName: "Incorporated Date (SSM Profile)" },
    { name: "businessCommencementDate", displayName: "Business Commencement Date" },
    { name: "businessNature", displayName: "Business Nature" },
    { name: "companyStatus", displayName: "Company Status (SSM)" },
    { name: "shareholders", displayName: "Shareholders" },
    { name: "directors", displayName: "Directors" },
    { name: "previousDirectors", displayName: "Previous Directors" },
    { name: "totalShareIssued", displayName: "Total Shares Issued" },
    { name: "companyLastOldName", displayName: "Previous Company Name" },
    { name: "companyNameDateOfChange", displayName: "Company Name Change Date" },
    { name: "businessOrigin", displayName: "Business Origin" },
    { name: "registeredAddress", displayName: "Registered Address (SSM)" },
  ];

  it.each(SSM_V2_FIELDS)("$name exists as standalone text field in category 14", ({ name, displayName }) => {
    const field = findField(name);
    expect(field, `Field '${name}' must exist`).toBeDefined();
    expect(field!.type).toBe("text");
    expect(String(field!.category)).toBe("14");
    expect(field!.displayName).toBe(displayName);
  });

  it("all 14 SSM v2 standalone fields are present", () => {
    const fieldNames = getBaseFieldSpecs().map((f) => f.name);
    for (const { name } of SSM_V2_FIELDS) {
      expect(fieldNames).toContain(name);
    }
  });

  it("SSM v2 date fields have inputType date", () => {
    const dateFields = ["ssmIncorporatedDate", "businessCommencementDate", "companyNameDateOfChange"];
    for (const name of dateFields) {
      const field = findField(name);
      expect(field!.inputType, `${name} should have inputType 'date'`).toBe("date");
    }
  });
});

// ---------------------------------------------------------------------------
// SYS-2181: Standalone NRIC v2 text fields (category 15)
// ---------------------------------------------------------------------------

describe("NRIC v2 standalone text fields (SYS-2181)", () => {
  const NRIC_V2_TEXT_FIELDS = [
    { name: "icName", displayName: "Full Name (NRIC)" },
    { name: "icNumber", displayName: "IC Number (NRIC Extracted)" },
    { name: "icAddress", displayName: "Address (NRIC)" },
    { name: "icGender", displayName: "Gender (NRIC)" },
    { name: "icReligion", displayName: "Religion (NRIC)" },
    { name: "icDateOfBirth", displayName: "Date of Birth (NRIC)" },
    { name: "icPlaceOfBirth", displayName: "Place of Birth (NRIC)" },
    { name: "icNationality", displayName: "Nationality (NRIC)" },
    { name: "icRace", displayName: "Race (NRIC)" },
  ];

  it.each(NRIC_V2_TEXT_FIELDS)("$name exists as standalone text field in category 15", ({ name, displayName }) => {
    const field = findField(name);
    expect(field, `Field '${name}' must exist`).toBeDefined();
    expect(field!.type).toBe("text");
    expect(String(field!.category)).toBe("15");
    expect(field!.displayName).toBe(displayName);
  });

  it("all 9 NRIC v2 standalone text fields are present", () => {
    const fieldNames = getBaseFieldSpecs().map((f) => f.name);
    for (const { name } of NRIC_V2_TEXT_FIELDS) {
      expect(fieldNames).toContain(name);
    }
  });

  it("icDateOfBirth has inputType date", () => {
    const field = findField("icDateOfBirth");
    expect(field!.inputType).toBe("date");
  });
});

// ---------------------------------------------------------------------------
// SYS-2181: ic document upload field
// ---------------------------------------------------------------------------

describe("ic document upload field (SYS-2181)", () => {
  it("exists as a file-type field in category 13", () => {
    const field = findField("ic");
    expect(field, "Field 'ic' must exist").toBeDefined();
    expect(field!.type).toBe("file");
    expect(String(field!.category)).toBe("13");
  });

  it("has displayName 'Identity Card (NRIC)'", () => {
    const field = findField("ic")!;
    expect(field.displayName).toBe("Identity Card (NRIC)");
  });

  it("has ihs_column_names with NRIC extracted fields", () => {
    const field = findField("ic")!;
    const expected = [
      "icName", "icNumber", "icAddress", "icGender",
      "icReligion", "icDateOfBirth", "icPlaceOfBirth",
      "icNationality", "icRace",
    ];
    const actual = [...(field.ihs_column_names as string[])].sort();
...
// ---------------------------------------------------------------------------
// SYS-2181: New categories
// ---------------------------------------------------------------------------

describe("new categories (SYS-2181)", () => {
  it("category 14 (SSM Company Profile Data) exists", () => {
    const categories = BASE_FIELD_SPECS.categories;
    const cat14 = categories.find((c) => c.id === "14");
    expect(cat14).toBeDefined();
    expect(cat14!.name).toBe("SSM Company Profile Data");
  });

  it("category 15 (NRIC Data) exists", () => {
    const categories = BASE_FIELD_SPECS.categories;
    const cat15 = categories.find((c) => c.id === "15");
    expect(cat15).toBeDefined();
    expect(cat15!.name).toBe("NRIC Data");
  });
});

// ---------------------------------------------------------------------------
// SYS-2181: idNumber vs icNumber disambiguation
// ---------------------------------------------------------------------------

describe("idNumber vs icNumber disambiguation (SYS-2181)", () => {
  it("idNumber displayName clarifies it is manual entry", () => {
    const field = findField("idNumber");
    expect(field).toBeDefined();
    expect(field!.displayName).toBe("IC Number (Manual Entry)");
  });

  it("icNumber displayName clarifies it is NRIC extracted", () => {
    const field = findField("icNumber");
    expect(field).toBeDefined();
    expect(field!.displayName).toBe("IC Number (NRIC Extracted)");
  });

  it("idNumber and icNumber are in different categories", () => {
    const idField = findField("idNumber")!;
    const icField = findField("icNumber")!;
    expect(idField.category).not.toBe(icField.category);
  });
});

// ---------------------------------------------------------------------------
// SYS-2181: getBaseFieldNames includes standalone fields
// ---------------------------------------------------------------------------

describe("getBaseFieldNames includes standalone SSM/NRIC fields (SYS-2181)", () => {
  const STANDALONE_FIELDS = [
    "ssmCompanyName", "ssmCompanyRegNo", "ssmIncorporatedDate",
    "businessCommencementDate", "businessNature", "companyStatus",
    "shareholders", "directors", "previousDirectors", "totalShareIssued",
    "companyLastOldName", "companyNameDateOfChange", "businessOrigin",
    "registeredAddress", "icName", "icNumber", "icAddress", "icGender",
    "icReligion", "icDateOfBirth", "icPlaceOfBirth", "icNationality", "icRace",
  ];

  it("returns all standalone SSM/NRIC field names", () => {
    const names = getBaseFieldNames();
    for (const name of STANDALONE_FIELDS) {
      expect(names.includes(name), `getBaseFieldNames() must include '${name}'`).toBe(true);
    }
  });
});

// ---------------------------------------------------------------------------
// SYS-2181: getBaseFieldSpecMap includes standalone fields
// ---------------------------------------------------------------------------

describe("getBaseFieldSpecMap includes standalone SSM/NRIC fields (SYS-2181)", () => {
  it("contains ic file field", () => {
    const map = getBaseFieldSpecMap();
    expect(map.has("ic")).toBe(true);
    expect(map.get("ic")!.type).toBe("file");
  });

  it("contains ssmCompanyName text field", () => {
    const map = getBaseFieldSpecMap();
    expect(map.has("ssmCompanyName")).toBe(true);
    expect(map.get("ssmCompanyName")!.type).toBe("text");
  });

  it("contains icNumber text field", () => {
    const map = getBaseFieldSpecMap();
    expect(map.has("icNumber")).toBe(true);
    expect(map.get("icNumber")!.type).toBe("text");
  });
});
