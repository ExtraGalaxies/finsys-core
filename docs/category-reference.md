...
| `mgmtCurrencySource` | string | enum |  | Where mgmtCurrency came from. `printed` when the statement prints it (a heading, a footer, or a currency prefix on its figures); `inferred` when it prints no currency anywhere and the application is Malaysian, so the writer stored MYR — shown so a reviewer knows the currency was not on the page. Absent on rows written before this field existed. |
