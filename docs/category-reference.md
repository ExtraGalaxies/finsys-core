...
| `shareholdingInterests` | string |  |  | JSON-encoded list of the companies the subject holds an interest or position in, one entry per printed row (name, registration number, incorporation date, paid-up capital, activity, position, appointment date, shareholding, percentage, remark). |
...
| `directorsAndOfficers` | string |  |  | JSON-encoded list of directors and officers from the corporation-details section, one entry per printed row (name, appointment date, designation). |
| `shareholdersAndMembers` | string |  |  | JSON-encoded list of shareholders from the corporation-details section, one entry per printed row (name, shareholding, percentage), the closing total row excluded. |
...
| `outstandingCreditFacilities` | string |  |  | JSON-encoded list of the outstanding banking credit facilities, one entry per printed line as the source returns them (an account row followed by its facility row, with any collateral attached), including the twelve monthly conduct-of-account marks. |
| `creditApplications` | string |  |  | JSON-encoded list of banking credit applications, one entry per printed line as the source returns them. |
| `specialAttentionAccounts` | string |  |  | JSON-encoded list of special-attention accounts, one entry per printed line as the source returns them. |
...
| `suitsAsDefendant` | string |  |  | JSON-encoded list of legal suits with the subject as defendant, one entry per suit. |
| `limitedDetailSuitsAsDefendant` | string |  |  | JSON-encoded list of other known suits, with limited details, with the subject as defendant. |
| `suitsAsPlaintiff` | string |  |  | JSON-encoded list of legal suits with the subject as plaintiff, one entry per suit. |
| `windingUpActionsAsDefendant` | string |  |  | JSON-encoded list of winding-up actions against the subject, one entry per action. |
| `windingUpActionsAsPetitioner` | string |  |  | JSON-encoded list of winding-up actions brought by the subject, one entry per action. |
| `bankruptcyActions` | string |  |  | JSON-encoded list of bankruptcy cases, one entry per case. |
| `bankruptcyActionCreditors` | string |  |  | JSON-encoded list of the creditors named in the bankruptcy cases (name and IC / passport / registration number). |
| `tradeCreditReferences` | string |  |  | JSON-encoded list of trade / credit references lodged against the subject, one entry per reference (creditor, amount due, days overdue, debt type, status, …). |
| `nonBankLenderFacilities` | string |  |  | JSON-encoded list of non-bank lender credit facilities, one entry per printed line as the source returns them, including the twelve monthly conduct-of-account marks. |
...
| `mgmtCurrency` | string |  |  | Currency as printed on the statement (e.g. RM, MYR). Null when the statement prints none. |
...
| `mgmtSalesRevenueItems` | string |  |  | JSON-encoded list of the account lines the statement prints under sales, revenue and turnover, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtCostOfGoodsSoldItems` | string |  |  | JSON-encoded list of the account lines the statement prints under cost of sales (purchases, opening and closing stock, direct labor, freight and other direct costs), for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtOtherIncomeItems` | string |  |  | JSON-encoded list of the account lines the statement prints under other operating and non-operating income, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtSalariesWagesItems` | string |  |  | JSON-encoded list of the account lines the statement prints under salaries, wages, statutory contributions and owners' or directors' remuneration, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtRentRatesItems` | string |  |  | JSON-encoded list of the account lines the statement prints under rental of premises and equipment, quit rent and assessment, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtUtilitiesItems` | string |  |  | JSON-encoded list of the account lines the statement prints under utilities (electricity, water, gas, sewerage), for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtTelephoneInternetItems` | string |  |  | JSON-encoded list of the account lines the statement prints under telephone, internet, hosting and subscriptions, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtMotorVehicleItems` | string |  |  | JSON-encoded list of the account lines the statement prints under motor-vehicle running costs, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtRepairsMaintenanceItems` | string |  |  | JSON-encoded list of the account lines the statement prints under repairs, upkeep and maintenance, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtInsuranceItems` | string |  |  | JSON-encoded list of the account lines the statement prints under insurance premiums, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtProfessionalFeesItems` | string |  |  | JSON-encoded list of the account lines the statement prints under accounting, audit, secretarial, tax, legal and consultancy fees, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtTravelEntertainmentItems` | string |  |  | JSON-encoded list of the account lines the statement prints under travel, accommodation, entertainment and meals, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtStaffCostsOtherItems` | string |  |  | JSON-encoded list of the account lines the statement prints under other staff costs (welfare, allowances, medical, training, recruitment, levy), for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtDepreciationItems` | string |  |  | JSON-encoded list of the account lines the statement prints under depreciation and amortization, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtOtherExpensesItems` | string |  |  | JSON-encoded list of the account lines the statement prints under every other expense account, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtTradeReceivablesItems` | string |  |  | JSON-encoded list of the account lines the statement prints under trade debtors / trade receivables, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtOtherDebtorsItems` | string |  |  | JSON-encoded list of the account lines the statement prints under other debtors and receivables, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtCashAtBankItems` | string |  |  | JSON-encoded list of the account lines the statement prints under bank accounts and fixed deposits, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtPettyCashItems` | string |  |  | JSON-encoded list of the account lines the statement prints under cash in hand / petty cash, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtDepositsPrepaymentsItems` | string |  |  | JSON-encoded list of the account lines the statement prints under deposits paid and prepayments, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtInventoryItems` | string |  |  | JSON-encoded list of the account lines the statement prints under stock, inventories and work in progress, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtLandBuildingItems` | string |  |  | JSON-encoded list of the account lines the statement prints under land and buildings (cost, accumulated depreciation and net book value rows), for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtRenovationsItems` | string |  |  | JSON-encoded list of the account lines the statement prints under renovations and other improvements, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtFurnitureFittingsItems` | string |  |  | JSON-encoded list of the account lines the statement prints under furniture, fittings and office equipment, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtVehiclesItems` | string |  |  | JSON-encoded list of the account lines the statement prints under motor vehicles and other vehicles, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtEquipmentToolsItems` | string |  |  | JSON-encoded list of the account lines the statement prints under plant, machinery, tools and other equipment, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtComputerEquipmentItems` | string |  |  | JSON-encoded list of the account lines the statement prints under computers and IT hardware, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtSystemSoftwareItems` | string |  |  | JSON-encoded list of the account lines the statement prints under software, systems and licenses capitalized as assets, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtTradePayablesItems` | string |  |  | JSON-encoded list of the account lines the statement prints under trade creditors / accounts payable, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtOtherCreditorsItems` | string |  |  | JSON-encoded list of the account lines the statement prints under other creditors and payables, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtAccrualsItems` | string |  |  | JSON-encoded list of the account lines the statement prints under accruals, accrued expenses and accrued wages, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtHirePurchaseItems` | string |  |  | JSON-encoded list of the account lines the statement prints under hire purchase and lease creditors, current and non-current, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtLoansFinancingItems` | string |  |  | JSON-encoded list of the account lines the statement prints under bank loans, term loans, overdrafts, SME financing and other alternative or peer-to-peer financing, current and non-current, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtShareCapitalItems` | string |  |  | JSON-encoded list of the account lines the statement prints under share capital and owners' or partners' capital, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtRetainedEarningsItems` | string |  |  | JSON-encoded list of the account lines the statement prints under retained earnings and accumulated profits or losses, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
| `mgmtDrawingsItems` | string |  |  | JSON-encoded list of the account lines the statement prints under owners' or partners' drawings and dividends, for this period: one entry per account with its code (when printed), its term as printed, and its amount. |
